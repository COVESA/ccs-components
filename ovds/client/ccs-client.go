...
fmt.Printf("writeToDataLake: Response= %s \n", jsonizedResponse)
	...
	url := "http://" + datalakeUrl + ":8765/datalakeserver"
...
func main() {
        ...
        if (len(os.Args) != 7) {
            fmt.Printf("CCS client command line: ./ccs-client nodelist-filename gen2-server-url datalake-server-url vss-tree-filename vin sleeptime\n")
            os.Exit(1)
        }
